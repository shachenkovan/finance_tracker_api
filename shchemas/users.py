    ...
    def date_of_birth_validate(cls, date_of_birth: date | None) -> date | None:
        ...
        if date_of_birth is not None:
            try:
                ...
                raise ValueError('Дата должна быть введена в формате YYYY-MM-DD.')
    ...
    def login_and_password_validate(cls, value: str | None) -> str | None:
        ...
        if value is not None:
            if not re.match(r"^[a-zA-Z0-9_.!$-]+$", value):
                raise ValueError('Логин и/или пароль могут содержать только латинские буквы, цифры и символы " _ ",  " . ",  " - ", " $ ", " ! "')
            if value.startswith(('_', '.', '-', '!', '$')):
                raise ValueError('Логин и/или пароль не могут начинаться с символов " _ ",  " . ",  " - ", " $ ", " ! "')
